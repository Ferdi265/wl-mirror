...
static void on_pw_core_info(void * data, const struct pw_core_info * info) {
    ...
    wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_core_info(): got core info: %p\n", info);
...
static void on_pw_core_done(void * data, uint32_t id, int seq) {
    ...
    wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_core_done(): got core done: id = %d, seq = %d\n", id, seq);
...
static void on_pw_core_error(void * data, uint32_t id, int seq, int res, const char * msg) {
    ...
    wlm_log_error("mirror-xdg-portal::on_pw_core_done(): got error: id = %d, seq = %d, res = %d, msg = %s\n", id, seq, res, msg);
    wlm_mirror_backend_fail_async(backend);
}
...
static void on_pw_renegotiate(void * data, uint64_t expirations) {
    ...
    wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_renegotiate(): need to renegotiate, expirations = %ld\n", expirations);
...
static void on_pw_process(void * data) {
    ...
    // TODO: actually do something here
    wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_process(): new video data\n");
...
static void on_loop_dbus_each(ctx_t * ctx) {
    ...
    if (old_fd != new_fd) {
        ...
        wlm_event_remove_fd(ctx, &handler);
