...
static void on_pw_core_info(void * data, const struct pw_core_info * info) {
    ...
    if (sscanf(info->version, "%d.%d.%d", &major, &minor, &patch) != 3) {
        wlm_log_error("mirror-xdg-portal::on_pw_core_info(): failed to parse pipewire version\n");
        wlm_mirror_backend_fail_async(backend);
        return;
    }
...
static void on_pw_stream_process(void * data) {
    ...
    if (backend->state != STATE_RUNNING) {
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_stream_process(): called while not running\n");
        return;
    }
    ...
    if (buffer == NULL) {
        wlm_log_error("mirror-xdg-portal::on_pw_stream_process(): out of buffers\n");
        wlm_mirror_backend_fail_async(backend);
        return;
    }
    ...
    if (meta != NULL) {
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_stream_process(): received meta header\n");

        if ((meta->flags & SPA_META_HEADER_FLAG_CORRUPTED) != 0) {
            wlm_log_error("mirror-xdg-portal::on_pw_stream_process(): received corrupt buffer\n");
            pw_stream_queue_buffer(backend->pw_stream, buffer);
            wlm_mirror_backend_fail_async(backend);
            return;
        }
    ...
    if (spa_buffer->datas[0].type == SPA_DATA_DmaBuf) {
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_stream_process(): received DMA buf\n");

        if (spa_buffer->n_datas > MAX_PLANES) {
            wlm_log_error("mirror-xdg-portal::on_pw_stream_process(): max %d planes are supported, got %d planes\n", MAX_PLANES, spa_buffer->n_datas);
            pw_stream_queue_buffer(backend->pw_stream, buffer);
            wlm_mirror_backend_fail_async(backend);
            return;
        }

        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_stream_process(): w=%d h=%d gl_format=%x drm_format=%c%c%c%c drm_modifier=%016lx\n",
        ...
        if (dmabuf.fds == NULL || dmabuf.offsets == NULL || dmabuf.strides == NULL) {
            wlm_log_error("mirror-xdg-portal::on_pw_stream_process(): failed to allocate dmabuf storage\n");
            pw_stream_queue_buffer(backend->pw_stream, buffer);
            wlm_mirror_backend_fail_async(backend);
            return;
        }
        ...
        for (size_t i = 0; i < dmabuf.planes; i++) {
            ...
            wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_stream_process(): fd=%d offset=% 10d stride=% 10d type=%d\n",
                dmabuf.fds[i], dmabuf.offsets[i], dmabuf.strides[i], spa_buffer->datas[i].type
            );
        }

        if (corrupted) {
            wlm_log_error("mirror-xdg-portal::on_pw_stream_process(): received corrupt dmabuf\n");
            pw_stream_queue_buffer(backend->pw_stream, buffer);
            wlm_mirror_backend_fail_async(backend);
            free(dmabuf.fds);
            free(dmabuf.offsets);
        ...
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_stream_process(): received SHM buf\n");

        wlm_log_error("mirror-xdg-portal::on_pw_stream_process(): SHM buffers not yet implemented\n");
        wlm_mirror_backend_fail_async(backend);
    } else {
        wlm_log_error("mirror-xdg-portal::on_pw_stream_process(): received unknown buffer type\n");
        wlm_mirror_backend_fail_async(backend);
    }
    ...
    if (region != NULL && spa_meta_region_is_valid(region)) {
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_stream_process(): received meta region\n");
        // TODO: handle region
    }
    ...
    if (transform != NULL) {
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_stream_process(): received meta transform\n");
...
static void on_pw_param_changed(void * data, uint32_t id, const struct spa_pod * param) {
    ...
    if (backend->state != STATE_RUNNING && backend->state != STATE_PW_CREATE_STREAM) {
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_param_changed(): called while not running\n");
        return;
    }

    if (id == SPA_PARAM_Format) {
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_param_changed(): format changed\n");

        uint32_t media_type;
        uint32_t media_subtype;
        if (spa_format_parse(param, &media_type, &media_subtype) < 0) {
            wlm_log_error("mirror-xdg-portal::on_pw_param_changed(): failed to parse SPA format\n");
            wlm_mirror_backend_fail_async(backend);
            return;
        }

        if (media_type != SPA_MEDIA_TYPE_video && media_subtype != SPA_MEDIA_SUBTYPE_raw) {
            wlm_log_error("mirror-xdg-portal::on_pw_param_changed(): unsupported media type '%d.%d'\n", media_type, media_subtype);
            wlm_mirror_backend_fail_async(backend);
            return;
        }
        ...
        if (spa_format_video_raw_parse(param, &info_raw) < 0) {
            wlm_log_error("mirror-xdg-portal::on_pw_param_changed(): failed to parse SPA raw format\n");
            wlm_mirror_backend_fail_async(backend);
            return;
        }
        ...
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_param_changed(): w=%d h=%d gl_format=%x drm_format=%c%c%c%c drm_modifier=%016lx\n",
            backend->w, backend->h, backend->gl_format,
            (backend->drm_format >> 0) & 0xFF,
            (backend->drm_format >> 8) & 0xFF,
            (backend->drm_format >> 16) & 0xFF,
        ...
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_param_changed(): spa_format=%d framerate=%d/%d\n",
        ...
        wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_param_changed(): unknown param id = %d\n", id);
    }
...
static void on_pw_state_changed(void * data, enum pw_stream_state old, enum pw_stream_state new, const char * error) {
    ...
    wlm_log_debug(ctx, "mirror-xdg-portal::on_pw_state_changed(): state = %s -> %s, error = %s\n", old_s, new_s, error);
