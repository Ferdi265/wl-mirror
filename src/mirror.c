...
static void dmabuf_frame_event_frame(
    ...
    log_debug("dmabuf_frame: received %dx%d frame with %d objects\n", width, height, num_objects);
...
static void dmabuf_frame_event_object(
    ...
    ctx_t * ctx = (ctx_t *)data;
    log_debug("dmabuf_frame: received %d byte object with plane_index %d\n", size, plane_index);
...
static void dmabuf_frame_event_ready(
    ...
    ctx_t * ctx = (ctx_t *)data;
    log_debug("dmabuf_frame: frame is ready\n");
    ...
    if (ctx->mirror->frame_image != EGL_NO_IMAGE) {
        log_debug("dmabuf_frame: destroying old EGL image\n");
        eglDestroyImage(ctx->egl->display, ctx->mirror->frame_image);
    }

    log_debug("dmabuf_frame: creating EGL image from dmabuf\n");
    int i = 0;
    ...
    log_debug("dmabuf_frame: binding image to EGL texture\n");
...
static void dmabuf_frame_event_cancel(
    ...
    ctx_t * ctx = (ctx_t *)data;
    log_debug("dmabuf_frame: frame was canceled\n");
    ...
    switch (reason) {
        ...
        case ZWLR_EXPORT_DMABUF_FRAME_V1_CANCEL_REASON_TEMPORARY:
            log_debug("dmabuf_frame: temporary cancellation\n");
            break;

        case ZWLR_EXPORT_DMABUF_FRAME_V1_CANCEL_REASON_RESIZING:
            log_debug("dmabuf_frame: cancellation due to output resize\n");
            break;
    ...
    log_debug("dmabuf_frame: closing files\n");
    for (unsigned int i = 0; i < ctx->mirror->num_objects; i++) {
...
static void frame_callback_event_done(
    ...
    log_debug("frame_callback: requesting next callback\n");
    ...
    log_debug("frame_callback: rendering frame\n");
    draw_texture_egl(ctx);

    log_debug("frame_callback: swapping buffers\n");
    ...
    if (ctx->mirror->state != STATE_WAIT_FRAME) {
        log_debug("frame_callback: clearing dmabuf_frame state\n");
        ctx->mirror->width = 0;
        ctx->mirror->height = 0;
        ctx->mirror->x = 0;
        ctx->mirror->y = 0;
        ...
        log_debug("frame_callback: creating wlr_dmabuf_export_frame\n");
        ...
        log_debug("frame_callback: adding dmabuf_frame event listener\n");
...
void init_mirror(ctx_t * ctx, char * output) {
    log_debug("init_mirror: allocating context structure\n");
    ...
    log_debug("init_mirror: searching for output\n");
    ...
    if (ctx->mirror->current == NULL) {
        ...
        log_debug("init_mirror: found output with name %s\n", output);
    }

    log_debug("init_mirror: formatting window title\n");
    char * title = NULL;
    ...
    log_debug("init_mirror: setting window title\n");
    xdg_toplevel_set_title(ctx->wl->xdg_toplevel, title);
    free(title);

    log_debug("init_mirror: requesting render callback\n");
    ctx->mirror->frame_callback = wl_surface_frame(ctx->wl->surface);
    wl_callback_add_listener(ctx->mirror->frame_callback, &frame_callback_listener, (void *)ctx);
}
...
void cleanup_mirror(ctx_t *ctx) {
    ...
    log_debug("cleanup_mirror: destroying mirror objects\n");
